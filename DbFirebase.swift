...
class DbFirebase: Database {
    ...
    var parentNotification: ((Todo?, DbAction?) -> Void)? // PlanGroupViewController에서 설정
    var existQuery: ListenerRegistration?                 // 이미 설정한 Query의 존재여부

    required init(parentNotification: ((Todo?, DbAction?) -> Void)?) {
        self.parentNotification = parentNotification
        reference = Firestore.firestore().collection("todos") // 첫번째 "plans"라는 Collection
    }
...
extension DbFirebase{
    func saveChange(todo: Todo, action: DbAction){
        if action == .Delete{
            reference.document(todo.key).delete()    // key로된 plan을 지운다
            return
        }
        // plan을 아카이빙한다.
//        var data = try? NSKeyedArchiver.archivedData(withRootObject: plan, requiringSecureCoding: false)
        let dict = todo.toDict().compactMapValues{$0}
        reference.document(todo.key).setData(dict)
...
extension DbFirebase{
    func onChangingData(querySnapshot: QuerySnapshot?, error: Error?){
        ...
        for documentChange in querySnapshot.documentChanges {
            ...
            let todo = Todo()
            todo.toTodo(dict: data as? [String:Any] ?? [:])
...
//            let plan = try? NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(data["data"] as! Data) as? Plan
            ...
            if let parentNotification = parentNotification {parentNotification(todo, action)} // 부모에게 알림
        }
    }
}
extension DbFirebase {
    func deleteTodo(todo: Todo) {
        reference.document(todo.key).delete { error in
            if let error = error {
                print("Error deleting todo from Firebase: \(error.localizedDescription)")
            } else {
                print("Todo deleted from Firebase successfully")
            }
        }
    }
}
